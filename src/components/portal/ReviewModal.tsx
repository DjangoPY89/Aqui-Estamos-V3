...
import { Star, X, CheckCircle2, Sparkles, RefreshCw, ShieldCheck } from "lucide-react";
  ...
  return (
    ...
    >
      <div
        ...
        className="my-auto bg-white/95 backdrop-blur-2xl rounded-2xl sm:rounded-3xl max-w-lg w-full max-h-[calc(100dvh-20px)] sm:max-h-[calc(100dvh-40px)] overflow-y-auto p-5 sm:p-7 shadow-2xl border border-white/60 relative space-y-4"
      >
        ...
        {isSuccess ? (
          <div className="text-center py-10 space-y-4 animate-in fade-in">
            ...
            <div className="space-y-1">
              <h3 className="text-xl font-black text-slate-950">¡Calificación Registrada!</h3>
              <p className="text-xs text-slate-500 leading-relaxed max-w-xs mx-auto">
                Tu valoración a <strong>{assignedCleaner}</strong> nos ayuda a reconocer el buen desempeño de nuestro personal con seguro IPS.
              </p>
            </div>
            <div className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-amber-50 text-amber-900 rounded-full text-xs font-bold border border-amber-200/60">
          ...
          <form onSubmit={onSubmit} className="space-y-5">
            ...
            {/* Header del Modal */}
            <div className="space-y-1">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-50 text-amber-800 rounded-full text-[11px] font-bold border border-amber-200/60">
                <Sparkles className="w-3.5 h-3.5 text-amber-500" />
                <span>Evaluación de Desempeño</span>
              </div>
              <h3 className="text-xl font-black text-slate-950 tracking-tight">
                Calificar a la Profesional
              </h3>
            ...
            <div className="py-4 px-3 text-center space-y-2.5 bg-slate-50 rounded-2xl border border-slate-200/60">
              ...
              <div className="flex items-center justify-center gap-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button
                    ...
                    className="p-1 transition-all hover:scale-125 active:scale-95 focus:outline-none cursor-pointer"
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs text-slate-900 focus:bg-white focus:ring-2 focus:ring-[#0071E3] focus:outline-none transition-all placeholder:text-slate-400 font-medium"
              />
            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              <button
                ...
                className="flex-1 py-3 bg-[#0071E3] hover:bg-[#0077ED] text-white rounded-full text-xs font-semibold flex items-center justify-center gap-2 transition-all shadow-xs active:scale-98 disabled:opacity-50 cursor-pointer"
              >
                {isSubmitting ? (
                  <>
              ...
              <button
                type="button"
                onClick={onClose}
                className="py-3 px-5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-full text-xs font-semibold transition-all active:scale-98 cursor-pointer"
              >
                Cancelar
              </button>
            </div>
